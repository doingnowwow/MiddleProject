...
public class hotel_payCheck_controller implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		reserv_name = LoginSession.session.getMem_name();
...
//			 이제부터는 불러온 객체의 메서드를 호출해서 사용할 수 있다.
		...
		System.out.println(" 이미지 : " + hotelInfo.get(0).getImg());
		/**
		 * 상단부 기능  -- 이미지 및 주소
		 */
		...
		bank.setOnAction(e->{
	 	...
	 	reservButton.setOnAction(e->{
			...
			saveBook.setBook_person(reserv_name); ; //예약자 이름 1
			...
			saveBook.setBook_request(request);
...
//			saveDate = sdate.format(sysdate);
			...
			try {
				save_sDate = sdate.parse(sDate);
				save_eDate = sdate.parse(eDate);
			} catch (ParseException e2) {
				e2.printStackTrace();
			}
			System.out.println("시작날짜 : " + sDate);
			System.out.println("끝날짜 : " + eDate);
			...
			if(checkMileage){
				...
				savePoint.setUse_date(save_sDate);; // 사용날짜
				savePoint.setSave_date(null); //적립날짜
				...
				savePoint.setUse_date(null); // 사용날짜
				savePoint.setSave_date(save_eDate); //적립날짜
				savePoint.setPoint_contents("호텔 예약 포인트 적립"); //포인트 적립 이유
			}
			
			
			saveMem.setMem_no(memInfo.getMem_no());
			saveMem.setMem_point(res_insertMileage);
			...
			System.out.println("예약 정보"
				...
				LoginSession.session.setMem_point(res_insertMileage);
		 		
		 		try {
		 			//결제 정보 저장
					cs_book.insertBookReserv(saveBook);
					cs_point.insertPointReserv(savePoint);
					cs_mem.updategrade(saveMem);
					saveChk();//결제완료 확인창
