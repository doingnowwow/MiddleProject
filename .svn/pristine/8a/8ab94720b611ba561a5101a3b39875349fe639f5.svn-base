...
//		LoginSession.dogsession = dsv;
	...
	//	    String com_email = comList.get(0).getCom_email(); /// 여기서 해당 분양게시글 사업자 정보 가져올수있음
			...
			String sendEmail =memin.getMem_email();
			String name = memin.getMem_name();
		    Sendmail mail = new Sendmail();
		    mail.setrecipient(sendEmail);
			mail.setsubject(name+ "님의 분양신청이 완료되었습니다.");
			mail.setbody(memin.getMem_name() + "님의 분양 신청이 완료 되었습니다."
					+ " \r\n" + "신청하신 분양 강아지 이름은" + dogSaleList.get(0).getDog_name()
					+ " \r\n" + "강아지 성별은 " + dogSaleList.get(0).getDog_gender()+"입니다"
					+" \r\n" + "신청해주셔서 감사합니다"
					+" \r\n" + "사랑합니다.");
			try {
				mail.Send();
